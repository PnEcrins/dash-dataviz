"""Composant carte Leaflet interactive."""
...
def create_map_component(sites: List[Site], selected_site_id: Optional[int] = None) -> html.Div:
    ...
    # Créer les CircleMarkers pour chaque site
    markers = []
    for site in sites:
        if not site.geom:
            continue
        ...
        if geom.get("type") == "Point":
            ...
            # Pour un polygone, on prend le centroïde (premier point)
            coords = geom.get("coordinates", [[]])[0]
            if coords:
                coords = coords[0]
        elif geom.get("type") == "MultiPolygon":
            # Pour MultiPolygon, première coord du premier polygon
            coords = geom.get("coordinates", [[[[]]]])[0][0][0]

        if not coords or len(coords) < 2:
            continue

        # Inverser les coordonnées (GeoJSON est [lon, lat], Leaflet est [lat, lon])
        lat, lon = coords[1], coords[0]

        # Couleur selon aire_valid
